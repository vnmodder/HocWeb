    ...
    const onSave = async () => {
        ...
        if (props.mode == "create") {
            ...
            formData.append("name", selected.value.name);
            if (file.value) {
    ...
    const loadInt = async () => {
        ...
        const response = await productApi.getCategoryById(model.value.id);
        if (response && response.data.result.isSuccess) {
        const data = response.data.result.data;
        selected.value.id = data.id;
        selected.value.categoryID = data.categoryID;
        selected.value.Description = data.Description;
        selected.value.unitBrief = data.unitBrief;
        selected.value.unitPrice = data.unitPrice;
